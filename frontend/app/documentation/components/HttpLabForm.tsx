  ...
  return (
    <div className="space-y-4">
      ...
      {(a2aMethod === "message/send" || a2aMethod === "message/stream") && conversationHistory && conversationHistory.length > 0 && (
        <div className="p-4 bg-bestgov-green/5 border border-bestgov-green/20 rounded-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <span className="text-sm font-medium text-bestgov-green">
                Multi-turn Conversation Active
              </span>
            </div>
          </div>
          
          <div className="text-xs text-bestgov-green/70">
      ...
      {(a2aMethod === "message/send" || a2aMethod === "message/stream" || a2aMethod.startsWith("tasks/")) && (
        <div className="p-4 bg-bestgov-blue/5 border border-bestgov-blue/20 rounded-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <input
                type="checkbox"
                ...
                className="rounded bg-[#222] border-[#444] text-bestgov-blue focus:ring-bestgov-blue"
              />
              <label htmlFor="enableWebhooks" className="text-sm font-medium text-bestgov-blue">
          ...
          {enableWebhooks && (
            <div className="mt-3">
              ...
              />
              <div className="text-xs text-bestgov-blue/70 mt-1">
      ...
      {currentTaskId && (
        <div className="p-3 bg-[#1a1a1a] border border-bestgov-green/20 rounded-md">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm text-neutral-400">Current Task ID:</span>
              <span className="ml-2 text-bestgov-green font-mono text-sm">{currentTaskId}</span>
            </div>
          </div>
        </div>
      )}
              ...
              {attachedFiles.map((file, index) => (
                ...
                >
                  {isImageFile(file.type) ? (
                    <Image className="h-4 w-4 text-bestgov-green" />
                  ) : file.type === 'application/pdf' ? (
                    <FileText className="h-4 w-4 text-bestgov-green" />
                  ) : (
                    <File className="h-4 w-4 text-bestgov-green" />
                  )}
      ...
      <Button 
        ...
        disabled={isLoading}
        className="bg-bestgov-green text-black hover:bg-[#00cc7d] w-full mt-4"
      >
        {isLoading ? (
