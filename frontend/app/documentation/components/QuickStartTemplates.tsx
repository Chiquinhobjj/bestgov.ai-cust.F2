...
export function QuickStartTemplates({ onSelectTemplate }: QuickStartTemplatesProps) {
  ...
  const getMethodColor = (method: string) => {
    switch (method) {
      case 'message/send': return 'bg-bestgov-green/20 text-bestgov-green border-bestgov-green/30';
      case 'message/stream': return 'bg-bestgov-blue/20 text-bestgov-blue border-bestgov-blue/30';
  ...
  return (
    <Card className="bg-[#1a1a1a] border-[#333] text-white mb-6">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((template) => {
            ...
            return (
              <div
                key={template.id}
                className="bg-[#222] border border-[#444] rounded-lg p-4 hover:border-bestgov-green/50 transition-colors cursor-pointer group"
                onClick={() => onSelectTemplate(template)}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    <div className="bg-bestgov-green/20 p-2 rounded-lg">
                      <IconComponent className="h-4 w-4 text-bestgov-green" />
                    </div>
                    <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    size="sm" 
                    variant="ghost" 
                    className="text-bestgov-green hover:text-bestgov-green/70 hover:bg-bestgov-green/10 text-xs px-2 py-1 h-auto opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    Use Template
                  </Button>
                </div>
        ...
        <div className="mt-4 p-3 bg-bestgov-blue/10 border border-bestgov-blue/20 rounded-lg">
          <p className="text-bestgov-blue/70 text-xs">
