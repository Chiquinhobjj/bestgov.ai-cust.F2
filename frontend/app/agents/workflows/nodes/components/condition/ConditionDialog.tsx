...
const conditionTypes = [
  {
    ...
    icon: <Filter className="h-5 w-5 text-bestgov-blue" />,
    color: "bg-bestgov-blue/80/30 border-bestgov-blue/60/50",
  },
];
      ...
      <DialogContent className="bg-neutral-800 border-neutral-700 text-neutral-200 sm:max-w-[650px]">
                ...
                <div
                  ...
                  className={`flex items-center space-x-3 rounded-md border p-3 cursor-pointer transition-all ${
                    selectedType === type.id
                      ? "bg-bestgov-blue/80/30 border-bestgov-blue/80"
                      : "border-neutral-700 hover:border-bestgov-blue/60/50 hover:bg-neutral-700/50"
                  }`}
                  onClick={() => setSelectedType(type.id)}
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-bestgov-blue/80/40">
                    {type.icon}
                  </div>
                  ...
                  {selectedType === type.id && (
                    <Badge className="bg-bestgov-blue/80 text-neutral-100">Selected</Badge>
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-md bg-neutral-700/50 border border-neutral-600 p-3 mt-4">
                  ...
                  <div className="text-sm">
                    <span className="text-bestgov-blue font-medium">{getFieldLabel(selectedField)}</span>
                    {" "}
                    <span className="text-neutral-300">{getOperatorLabel(selectedOperator)}</span>
                    {" "}
                    {!["is_defined", "is_not_defined"].includes(selectedOperator) && (
                      <span className="text-bestgov-green font-medium">"{comparisonValue || "(empty)"}"</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            className="bg-bestgov-blue/60 hover:bg-bestgov-blue/80 text-white"
          >
            Add Condition
          </Button>
        </DialogFooter>
