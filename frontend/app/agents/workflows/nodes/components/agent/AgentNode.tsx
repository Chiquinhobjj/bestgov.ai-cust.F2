...
export function AgentNode(props: NodeProps) {
  ...
  const getAgentTypeIcon = (type: string) => {
    switch (type) {
      case "llm":
        return <Code className="h-4 w-4 text-bestgov-green" />;
      case "a2a":
      ...
      case "workflow":
        return <Workflow className="h-4 w-4 text-bestgov-blue" />;
      case "task":
  ...
  const getModelBadgeColor = (model: string) => {
    if (model?.includes('gpt-4')) return 'bg-bestgov-blue/80/30 text-bestgov-green border-bestgov-green/80/30';
    ...
    if (model?.includes('gemini')) return 'bg-bestgov-blue/80/30 text-bestgov-blue border-bestgov-blue/80/30';
    ...
    <BaseNode hasTarget={true} selected={selected || false} borderColor="blue" isExecuting={isExecuting}>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-bestgov-blue/80/40 shadow-sm">
            <User className="h-5 w-5 text-bestgov-blue" />
          </div>
          <div>
            <p className="text-lg font-medium text-bestgov-blue">
      ...
      {agent ? (
        <div className="mb-3 rounded-lg border border-bestgov-blue/60/40 bg-bestgov-blue/90/10 p-3 transition-all duration-200 hover:border-bestgov-blue/80/50 hover:bg-bestgov-blue/80/10">
          <div className="flex flex-col">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge
                variant="outline"
                className="px-1.5 py-0 text-xs bg-bestgov-blue/80/30 text-bestgov-blue border-bestgov-blue/60/40"
              >
        ...
        <div className="mb-3 flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-bestgov-blue/60/40 bg-bestgov-blue/90/10 p-5 text-center transition-all duration-200 hover:border-bestgov-blue/80/50 hover:bg-bestgov-blue/80/20">
          <User className="h-8 w-8 text-bestgov-blue/60/50 mb-2" />
          <p className="text-bestgov-blue">Select an agent</p>
      ...
      <div className="mt-2 flex items-center justify-end text-sm text-neutral-400 transition-colors">
        ...
        <Handle
          className={cn(
            ...
            isBottomHandleConnected ? "!bg-bestgov-blue !border-bestgov-blue" : "!bg-neutral-400 !border-neutral-500",
            selected && isBottomHandleConnected && "!bg-bestgov-blue !border-bestgov-blue/70"
          )}
          style={{
            right: "-8px",
            top: "calc(100% - 25px)",
