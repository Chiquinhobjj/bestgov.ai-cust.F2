  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] flex flex-col overflow-hidden bg-background border-border">
          <DialogHeader className="flex flex-row items-start justify-between pb-2">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className="bg-card border-border text-accent"
              >
              ...
              <Button
                variant="outline"
                size="icon"
                className="h-8 w-8 rounded-full bg-card border-border hover:bg-accent/20 hover:text-accent"
          ...
          >
            <TabsList className="bg-muted/50 p-1 border-b border-border">
              <TabsTrigger
                value="info"
              ...
              <TabsContent value="info" className="mt-0 space-y-4">
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-card p-3 rounded-md border border-border flex flex-col items-center justify-center text-center">
                      <Code className="h-5 w-5 text-accent mb-1" />
                    ...
                    <div className="bg-card p-3 rounded-md border border-border flex flex-col items-center justify-center text-center">
                      <TagIcon className="h-5 w-5 text-accent mb-1" />
                  ...
                  <div className="bg-neutral-800 p-4 rounded-md border border-neutral-700">
                    <h3 className="text-sm font-medium text-bestgov-green mb-2">
                      Agent Role
                    </h3>
                      ...
                      <div className="bg-neutral-800 p-4 rounded-md border border-neutral-700">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {agent.config.tools.map((tool, index) => (
                            ...
                            >
                              <TagIcon className="h-3.5 w-3.5 mr-1.5 text-bestgov-green" />
                          ...
                          {agent.config.agent_tools.map((agentId, index) => (
                            ...
                            >
                              <div className="flex items-center">
                                <Bot className="h-3.5 w-3.5 mr-2 text-bestgov-green" />
                          ...
                          <div className="space-y-2">
                            {agent.config.custom_tools.http_tools.map(
                              (tool, index) => (
                                ...
                                >
                                  <div className="flex items-center">
                                    <Server className="h-3.5 w-3.5 mr-2 text-bestgov-green" />
                                    <span className="text-sm text-neutral-300">
                                      {tool.name}
                                    </span>
                                  </div>
                                  <Badge
                                    variant="outline"
                                    className="text-xs bg-neutral-800 border-neutral-700 text-bestgov-green"
                                  >
                                    {tool.method}
                                  </Badge>
                                </div>
                      ...
                      <div className="space-y-2">
                        {agent.config?.mcp_servers &&
                          agent.config.mcp_servers.map((mcp, index) => (
                            ...
                            >
                              <Server className="h-3.5 w-3.5 mr-2 text-accent" />
