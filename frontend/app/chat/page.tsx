      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {selectedSession || currentAgentId ? (
                ...
                return (
                  <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                      <div className="p-1 rounded-full bg-bestgov-green/20">
                    ...
                    <div className="flex items-center gap-2">
                      {currentAgent && (
                        <Badge 
                          className="bg-bestgov-green text-bestgov-blue px-3 py-1 text-sm border-0 cursor-pointer hover:bg-bestgov-green/90 transition-colors"
            ...
            >
              {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-bestgov-green to-bestgov-blue flex items-center justify-center shadow-lg mb-4 relative">
                    <Loader2 className="h-6 w-6 text-white animate-spin" />
                    <div className="absolute inset-0 rounded-full blur-md bg-bestgov-green/20 animate-pulse"></div>
                  </div>
                  <p className="text-neutral-400 mb-2">Loading conversation...</p>
                </div>
              ) : messages.length === 0 ? (
                <div className="flex flex-col h-full items-center justify-center text-center p-6">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-bestgov-green/20 to-bestgov-blue/20 flex items-center justify-center shadow-lg mb-5 border border-bestgov-green/30">
                    <MessageSquare className="h-6 w-6 text-bestgov-green" />
                  </div>
                      ...
                      <div className="flex gap-3 max-w-[80%]">
                        ...
                        <div className="rounded-lg p-3 bg-gradient-to-br from-neutral-800 to-neutral-900 border border-neutral-700/50 shadow-md">
                          <div className="flex space-x-2">
                            <div className="h-2 w-2 rounded-full bg-bestgov-green animate-bounce"></div>
                            <div className="h-2 w-2 rounded-full bg-bestgov-green animate-bounce [animation-delay:0.2s]"></div>
                            <div className="h-2 w-2 rounded-full bg-bestgov-green animate-bounce [animation-delay:0.4s]"></div>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {isSending && !isLoading && (
                <div className="px-4 py-2 mb-3 rounded-lg bg-neutral-800/50 border border-neutral-700/30 text-sm text-neutral-400 flex items-center shadow-sm">
                  <div className="mr-2 relative">
                    <Loader2 className="h-3.5 w-3.5 animate-spin text-bestgov-green" />
                    <div className="absolute inset-0 blur-sm bg-bestgov-green/20 rounded-full animate-pulse"></div>
                  </div>
                  Agent is thinking...
                </div>
              )}
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-20 h-20 rounded-full bg-bestgov-green/20 flex items-center justify-center shadow-lg mb-6 border border-bestgov-green/30">
              <MessageSquare className="h-10 w-10 text-bestgov-green" />
            </div>
            ...
            <Button
              ...
              className="bg-bestgov-green text-bestgov-blue hover:bg-bestgov-green/90 px-6 py-6 h-auto shadow-md rounded-xl"
            >
        ...
        <DialogContent className="bg-neutral-900 border-neutral-800 text-white shadow-xl">
          <DialogHeader>
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 rounded-full bg-bestgov-green/20">
                <MessageSquare className="h-5 w-5 text-bestgov-green" />
              </div>
          ...
          <div className="mt-4 space-y-4">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 bg-neutral-800/40 border-neutral-700/50 text-white focus-visible:ring-bestgov-green/50 focus-visible:border-bestgov-green/50 shadow-inner rounded-xl"
                value={agentSearchTerm}
                onChange={(e) => setAgentSearchTerm(e.target.value)}
              />
              {agentSearchTerm && (
                <button
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 text-neutral-400 hover:text-bestgov-green transition-colors"
            ...
            <ScrollArea className="h-[300px] pr-2">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-8">
                  <div className="relative">
                    <Loader2 className="h-8 w-8 text-bestgov-green animate-spin" />
                    <div className="absolute inset-0 rounded-full blur-md bg-bestgov-green/20 animate-pulse"></div>
                  </div>
                ...
                <div className="space-y-2">
                  {filteredAgents.map((agent) => (
                    <div
                      key={agent.id}
                      className="p-3 rounded-md cursor-pointer transition-all bg-neutral-800 hover:bg-neutral-800/90 border border-neutral-700/30 hover:border-bestgov-green/30 shadow-sm hover:shadow-md group"
                      onClick={() => selectAgent(agent.id)}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div className="p-1 rounded-full bg-bestgov-green/20 group-hover:bg-bestgov-green/30 transition-colors">
                          <MessageSquare size={14} className="text-bestgov-green" />
                        </div>
                        <span className="font-medium text-white group-hover:text-bestgov-gray-light">
                          {agent.name}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-1">
                        <Badge className="text-xs bg-neutral-800/60 text-bestgov-green border border-bestgov-green/30">
                          {agent.type}
                        </Badge>
                        {agent.model && (
